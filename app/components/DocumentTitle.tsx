...
export function DocumentTitle() {
  ...
  } else { // TODO: instead edit title, make it as select from omega, torus_rec, torus_data and their version
    return (
      <updateDoc.Form method="post" action={`/actions/${doc.id}/update`}>
        ...
        >
          <label className="relative block group">
            <ChevronDownIcon className="h-5 w-5 absolute top-1/2 transform -translate-y-1/2 left-3 text-white opacity-0 transition pointer-events-none group-hover:opacity-80 group-focus:opacity-80" />
            <input
              ref={ref}
              className={
